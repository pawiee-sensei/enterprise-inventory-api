export function Switch({ checked, onChange, disabled = false, showLabel = false }) {
  return (
    <div className="flex items-center gap-2">
      ...
      {showLabel && (
        <span className={`text-xs font-medium ${checked ? "text-success" : "text-danger"}`}>
          {checked ? "On Sale" : "Off Sale"}
        </span>
      )}
    </div>
  );
